...
fi(iSeg) = subs(f,y,sym('(1+2/5*sqrt(5))/2'));
fi(iSeg) = subs(fi(iSeg),kf,2/5*sqrt(5)/TimeSeg(iSeg));
...
fi(iSeg) = subs(f,y,sym('(1+2/5*sqrt(5))/2-kf*t'));
fi(iSeg) = subs(fi(iSeg),kf,2/5*sqrt(5)/TimeSeg(iSeg));
intfi(iSeg) = int(fi(iSeg),t,0,t);
...
Sound15 = DataAudio;
save Sound15.mat Sound15 AudioSampleRate;
